...
class Mail
{
    ...
    public function ReplyTo($storeAddress)
    {
        if (!is_string($storeAddress))
            return false;
        $temp_mass = explode(';', $storeAddress);
        if (count($temp_mass) == 2)
        {
        ...
        else
        {
            ...
            $this->headers['Reply-To'] = $storeAddress;
        }
    }
    ...
    public function ValidEmail($storeAddress)
    {
        if (function_exists('filter_list'))
        {
            $valid_email = filter_var($storeAddress, FILTER_VALIDATE_EMAIL);
        ...
        {
            if (ereg(".*<(.+)>", $storeAddress, $regs))
            {
                $storeAddress = $regs[1];
            }
            if (ereg("^[^@  ]+@([a-zA-Z0-9\-]+\.)+([a-zA-Z0-9\-]{2}|net|com|gov|mil|org|edu|int)\$", $storeAddress))
                return true;
            else
                return false;
        }
